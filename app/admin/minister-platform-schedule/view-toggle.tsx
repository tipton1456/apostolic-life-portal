...
export type MinisterPlatformView = "calendar" | "list";

const VIEW_OPTIONS: Array<{ label: string; value: MinisterPlatformView }> = [
  { label: "Calendar View", value: "calendar" },
  { label: "List View", value: "list" },
];

export default function MinisterPlatformViewToggle({
  view,
}: {
  view: MinisterPlatformView;
}) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [pendingView, setPendingView] =
    useState<MinisterPlatformView | null>(null);
  const activeView = pendingView ?? view;
  const isNavigating = isPending || pendingView !== null;

  function handleViewChange(nextView: MinisterPlatformView) {
    if (nextView === view || isNavigating) return;

    setPendingView(nextView);
    startTransition(() => {
      router.push(`/admin/minister-platform-schedule?view=${nextView}`);
    });
  }
