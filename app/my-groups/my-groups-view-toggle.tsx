...
export type MyGroupsView = "elvanto" | "planning-center";

const VIEW_OPTIONS: Array<{ label: string; value: MyGroupsView }> = [
  { label: "Elvanto Groups", value: "elvanto" },
  { label: "Planning Center Teams", value: "planning-center" },
];

export default function MyGroupsViewToggle({ view }: { view: MyGroupsView }) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [pendingView, setPendingView] = useState<MyGroupsView | null>(null);
  const activeView = pendingView ?? view;
  const isNavigating = isPending || pendingView !== null;

  function handleViewChange(nextView: MyGroupsView) {
    if (nextView === view || isNavigating) return;

    setPendingView(nextView);
    startTransition(() => {
      router.push(`/my-groups?view=${nextView}`);
    });
  }
